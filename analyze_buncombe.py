...
# Load the CSV data
trails = []
with open('/Users/fredbliss/workspace/treasure/data/trails_summary.csv', 'r') as f:
...
for i, trail in enumerate(buncombe_trails[:20], 1):
    print(f"{i:2d}. {trail['name'][:50]:<50} {trail['distance_miles']:>6} mi | {trail['highway_type']:<8} | {trail['surface']:<12} | {trail['difficulty']}")
...
for hwy_type, count in highway_types.most_common():
    total_miles = sum(float(t['distance_miles']) for t in buncombe_trails if t['highway_type'] == hwy_type)
...
# Analyze difficulty
difficulties = Counter([t['difficulty'] for t in buncombe_trails if t['difficulty'] != 'unknown'])
...
for trail in fs_trails[:10]:
    print(f"  {trail['name'][:45]:<45} {trail['ref']:<10} {trail['distance_miles']:>6} mi")
...
for i, trail in enumerate(unknown_trails[:15], 1):
    print(f"{i:2d}. {trail['name'][:50]:<50} {trail['distance_miles']:>6} mi | {trail['highway_type']}")
...
print(f"Total Unknown miles: {sum(float(t['distance_miles']) for t in unknown_trails):.2f}")
