...
def find_tiles_near_center() -> List[Dict]:
    ...
    for item in metadata['items']:
        ...
        if (bbox['minX'] <= CENTER_LON <= bbox['maxX'] and
            bbox['minY'] <= CENTER_LAT <= bbox['maxY']):
            ...
            distance = ((tile_center_lat - CENTER_LAT)**2 +
                       (tile_center_lon - CENTER_LON)**2)**0.5
...
def main():
    ...
    if center_tiles:
        ...
        for i, tile_info in enumerate(center_tiles[:5], 1):  # Limit to 5 tiles
            ...
            print(f"  Bounding box: {item['boundingBox']['minY']:.3f}°N to {item['boundingBox']['maxY']:.3f}°N")
            print(f"               {item['boundingBox']['minX']:.3f}°W to {item['boundingBox']['maxX']:.3f}°W")
