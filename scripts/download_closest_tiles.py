...
def find_closest_tiles(max_tiles: int = 3) -> List[Dict]:
    ...
    for item in metadata['items']:
        ...
        overlaps = (bbox['minX'] - 0.05 <= CENTER_LON <= bbox['maxX'] + 0.05 and
                   bbox['minY'] - 0.05 <= CENTER_LAT <= bbox['maxY'] + 0.05)
    ...
    for i, tile_info in enumerate(tiles, 1):
        ...
        if success:
            ...
            if 'previewGraphicURL' in item and item['previewGraphicURL']:
                ...
                try:
                    preview_response = requests.get(item['previewGraphicURL'], timeout=30)
