...
def identify_access_restrictions(df):
    ...
    df["has_any_restriction"] = (
        df["has_private_access"] | df["has_private_motor_vehicle"]
    )
...
def main():
    ...
    print(
        f"  Trails with access=private: {private_access} ({private_access / total_trails * 100:.1f}%)"
    )
    print(
        f"  Trails with motor_vehicle=private: {private_motor_vehicle} ({private_motor_vehicle / total_trails * 100:.1f}%)"
    )
    print(
        f"  Trails with any restriction: {any_restriction} ({any_restriction / total_trails * 100:.1f}%)"
    )
    ...
    if removed_count > 0:
        ...
        print(
            removed[["trail_name", "osm_id", "total_score", "access", "motor_vehicle"]]
        )

    # Re-rank remaining trails
    print("\nStep 6: Re-ranking remaining trails...")
    public_df = public_df.sort_values("total_score", ascending=False).reset_index(
        drop=True
    )
