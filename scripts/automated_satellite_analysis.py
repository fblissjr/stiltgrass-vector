...
def download_google_static_map(lat, lon, zoom=18, size="640x640", maptype="satellite"):
    ...
    print(
        f"Alternative: Manually visit https://www.google.com/maps/@{lat},{lon},{zoom}z/data=!3m1!1e3"
    )
    return None
...
def analyze_trail_visibility(sat_image, reference_photos):
    ...
    matches_orientation = (
        len(ns_angles) > len(angles) * 0.3 or len(nwse_angles) > len(angles) * 0.3
    )
...
def compare_to_aerial_photos(sat_image, photo_nums=[4, 5, 6]):
    ...
    return {
        ...
        "match_quality": str(
            "HIGH" if best_score > 20 else "MEDIUM" if best_score > 10 else "LOW"
        ),
    }
...
def main():
    ...
    try:
        ...
        coordinates = [
            ...
            {
                "name": "Old Trestle Road Seg1",
                "lat": 35.6397,
                "lon": -82.2944,
                "rank": 2,
            },
            {
                "name": "Old Trestle Road Seg2",
                "lat": 35.6390,
                "lon": -82.2950,
                "rank": 3,
            },
    ...
    for trail in coordinates[:5]:
        ...
        if sat_image:
            ...
            output_file = (
                OUTPUT_DIR
                / f"rank{trail['rank']}_{trail['name'].replace(' ', '_')}_satellite.jpg"
            )
            ...
            results.append({**trail, **visibility, **comparison})
        else:
            print("✗ Could not download imagery")
            print(
                f"  Manual check: https://www.google.com/maps/@{trail['lat']},{trail['lon']},17z/data=!3m1!1e3"
            )
    ...
    if results:
        ...
        for r in sorted(results, key=lambda x: x["confidence"], reverse=True):
            ...
            print(
                f"  Recommendation: {'HIGH PRIORITY' if r['confidence'] > 70 else 'MEDIUM' if r['confidence'] > 50 else 'LOW'}"
            )
