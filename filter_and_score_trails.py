...
def has_japanese_stilt_grass_habitat(surface: str, difficulty: str, elevation: float) -> bool:
    """
...
def score_trail(feature: Dict) -> TrailScore:
    ...
    straight_distance = haversine_distance(CHARLOTTE_LON, CHARLOTTE_LAT, center_lon, center_lat)
...
def main():
    ...
    with open('/Users/fredbliss/workspace/treasure/data/trails.geojson', 'r') as f:
    ...
    for i, trail in enumerate(top_50[:10], 1):
        ...
        print(f"   Score: {trail.score} | Drive time: {trail.drive_time_minutes:.0f} min | Elevation: {trail.elevation_est:.0f} ft")
    ...
    for trail_score in filtered_trails:
        ...
        for feature in trails_data['features']:
            if str(feature['properties'].get('osm_id', '')) == trail_score.osm_id:
                ...
                feature['properties']['drive_time_minutes'] = trail_score.drive_time_minutes
    ...
    with open('/Users/fredbliss/workspace/treasure/data/filtered_trails.geojson', 'w') as f:
    ...
    with open('/Users/fredbliss/workspace/treasure/data/top_50_candidates.csv', 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow([
            'rank', 'name', 'score', 'osm_id', 'latitude', 'longitude',
            'elevation_ft', 'drive_time_min', 'distance_miles', 'county',
            'surface', 'difficulty', 'highway_type', 'ref',
            'score_surface', 'score_difficulty', 'score_buncombe', 'score_stilt_grass',
            'score_forest_service', 'score_named', 'score_cell', 'score_elevation'
        ])

        for i, trail in enumerate(top_50, 1):
            writer.writerow([
                i, trail.name, trail.score, trail.osm_id,
                f"{trail.latitude:.6f}", f"{trail.longitude:.6f}",
                f"{trail.elevation_est:.0f}", f"{trail.drive_time_minutes:.0f}",
                f"{trail.distance_miles:.2f}", trail.county,
                trail.surface, trail.difficulty, trail.highway_type, trail.ref,
    ...
    with open('/Users/fredbliss/workspace/treasure/data/top_50_candidates.geojson', 'w') as f:
    ...
    elevation_fails = sum(1 for t in all_scored_trails if not (MIN_ELEVATION <= t.elevation_est <= MAX_ELEVATION))
    drive_time_fails = sum(1 for t in all_scored_trails if not (MIN_DRIVE_TIME <= t.drive_time_minutes <= MAX_DRIVE_TIME))
    cell_coverage_fails = sum(1 for t in all_scored_trails if t.score_breakdown.get('cell_coverage', 0) < 3)
    ...
    difficulty_stats = {}
    for trail in filtered_trails:
        difficulty_stats[trail.difficulty] = difficulty_stats.get(trail.difficulty, 0) + 1
    ...
    with open('/Users/fredbliss/workspace/treasure/data/filter_statistics.json', 'w') as f:
    ...
    print()
    print("Files created:")
    print("  - /Users/fredbliss/workspace/treasure/data/filtered_trails.geojson")
    print("  - /Users/fredbliss/workspace/treasure/data/top_50_candidates.csv")
    print("  - /Users/fredbliss/workspace/treasure/data/top_50_candidates.geojson")
    print("  - /Users/fredbliss/workspace/treasure/data/filter_statistics.json")
    print()
    print(f"Top candidate: {top_50[0].name if top_50 else 'None'}")
    print(f"  Score: {top_50[0].score if top_50 else 0}")
    print(f"  Location: {top_50[0].latitude:.4f}, {top_50[0].longitude:.4f}" if top_50 else "")
