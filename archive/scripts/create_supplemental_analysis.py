...
for i in range(8):
    ...
    ax.set_title(f'Photo {photo_num}: {category}', fontweight='bold', color=color, fontsize=12)
    ax.axis('off')

plt.suptitle('Aerial Photo Progression: Ground Level to Canopy', fontsize=18, fontweight='bold')
...
axes[1, 0].bar(x - width/2, green_pcts, width, label='Green Vegetation', color='green', alpha=0.7)
axes[1, 0].bar(x + width/2, brown_pcts, width, label='Brown/Bare Ground', color='brown', alpha=0.7)
axes[1, 0].set_xlabel('Photo Number', fontsize=12)
axes[1, 0].set_ylabel('Coverage (%)', fontsize=12)
axes[1, 0].set_title('Vegetation vs Bare Ground Coverage', fontweight='bold', fontsize=14)
...
plt.tight_layout()
plt.savefig(OUTPUT_DIR / 'photo_measurements_analysis.png', dpi=150, bbox_inches='tight')
plt.close()
...
for idx, photo_num in enumerate([4, 5, 6]):
    ...
    # Draw annotations
    axes[idx].imshow(img)
    axes[idx].set_title(f'Photo {photo_num}: Trail Features', fontweight='bold', fontsize=14)
    axes[idx].axis('off')
...
plt.suptitle('Mid-Altitude Trail Photos: Key for Satellite Matching', fontsize=16, fontweight='bold')
plt.tight_layout()
