...
class PhotoAnalyzer:
    ...
    def load_photos(self):
        ...
        for i in range(1, 9):
            ...
            if path.exists():
                ...
                print(
                    f"  Loaded {filename}: {img.shape[1]}x{img.shape[0]} pixels (downsampled)"
                )
            else:
    ...
    def analyze_mid_altitude(self):
        ...
        for idx, photo_num in enumerate([4, 5, 6]):
            ...
            print(
                f"    {len(linear_features) if lines is not None else 0} lines, "
    ...
    def create_visual_signature(self):
        ...
        if self.results["color_profiles"]:
            ...
            if mid_alt_colors:
                ...
                avg_brown = np.mean(
                    [c["brown_bare_percentage"] for c in mid_alt_colors]
                )
    ...
    def create_comprehensive_report(self):
        ...
        md += (
            "Photos 4-6 provide the most valuable data for satellite matching, showing "
        )
