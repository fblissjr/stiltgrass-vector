...
def terrain_analysis():
    ...
    print(
        f"  Relief: {np.nanmax(dem) - np.nanmin(dem):.0f}m ({(np.nanmax(dem) - np.nanmin(dem)) * 3.28084:.0f}ft)"
    )
    print()
    ...
    if south_pixels > 0:
        ...
        print(
            f"  Elevation range: {np.nanmin(south_dem):.0f}m to {np.nanmax(south_dem):.0f}m"
        )
        print(
            f"  Mean elevation: {np.nanmean(south_dem):.0f}m ({np.nanmean(south_dem) * 3.28084:.0f}ft)"
        )
